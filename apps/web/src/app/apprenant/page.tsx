...
export default async function ApprenantPage() {
  ...
  const [
    ...
    supabase
      .from("enrollments")
      .select(
        "id, status, created_at, sessions(reference, starts_on, ends_on, trainings(id, title, slug))",
      )
    ...
    supabase
      .from("bookings")
      .select(
        "booking_date, start_time, reason, profiles!bookings_trainer_id_fkey(first_name, last_name)",
      )
      .eq("learner_id", user!.id)
      .eq("status", "confirmee")
  ...
  const [{ data: lessonRows }, { data: progressRows }] = await Promise.all([
    ...
    enrollmentIds.length
      ...
      : Promise.resolve({
          data: [] as { enrollment_id: string; completed_at: string | null }[],
        }),
  ]);
  ...
  for (const row of lessonRows ?? []) {
    lessonCountByTraining.set(
      row.training_id,
      (lessonCountByTraining.get(row.training_id) ?? 0) +
  ...
  const attendances = [...(attendancesRaw ?? [])].sort((a, b) => {
    const dateDiff = (a.session_slots?.slot_date ?? "").localeCompare(
      b.session_slots?.slot_date ?? "",
    );
  ...
  const inProgress = confirmedEnrollments
    .map((e) => {
      ...
      const total = trainingId
        ? (lessonCountByTraining.get(trainingId) ?? 0)
        : 0;
  ...
  const attendanceGroups = (() => {
    const map = new Map<
      string,
      { key: string; label: string; items: typeof attendances }
    >();
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <p className="mt-1 font-body text-sm text-foreground-muted">
            Un peu de curiosité, un pas de plus. Retrouvez votre élan.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          {/* Hero : formation à reprendre */}
          <div className="learner-resume flex flex-col justify-between overflow-hidden rounded-xl border border-accent/30 bg-primary p-6 text-on-primary lg:col-span-2">
            {inProgress ? (
              <>
                <div>
                  ...
                  <h2 className="font-display text-xl font-bold md:text-2xl">
                    {inProgress.enrollment.sessions?.trainings?.title ??
                      "Formation"}
                  </h2>
            ...
            <CardContent className="flex flex-col gap-3">
              {nextBooking ? (
                <div className="rounded-DEFAULT border border-accent/40 bg-surface-elevated p-3">
                  ...
                  {nextBooking.reason && (
                    <p className="mt-0.5 font-body text-xs text-foreground-muted line-clamp-1">
                      {nextBooking.reason}
                    </p>
                  )}
                </div>
          ...
          {!enrollments || enrollments.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {enrollments.map((enrollment) => {
                ...
                const canAccessProgramme = ["confirme", "termine"].includes(
                  enrollment.status,
                );
                const totalLessons = training?.id
                  ? (lessonCountByTraining.get(training.id) ?? 0)
                  : 0;
                const completedLessons =
                  completedCountByEnrollment.get(enrollment.id) ?? 0;
                return (
                  <div
                    key={enrollment.id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Badge
                        variant={
                          STATUS_VARIANTS[enrollment.status] ?? "neutral"
                        }
                      >
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-2">
            ...
            {unsignedCount > 0 && (
              <Badge variant="warning">{unsignedCount} à signer</Badge>
            )}
          </CardHeader>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {attendance.signed_at ? (
                              <Badge variant="success">
                                Signé le{" "}
                                {new Date(attendance.signed_at).toLocaleString(
                                  "fr-FR",
                                )}
                              </Badge>
                            ) : (
